...
extensions, GridTools, Dash, Array Extensions \cite{all} have arise to give developers more control over 
 ...
 In this chapter, we discuss the key points when designing such abstractions, emerging approaches with (I'm)mature
 solutions, and potential research areas. Here, we focus on help manage locality on data-parallel algorithms 
...
datum residing in one or more memory spaces (e.g. main memory, local storage). Thus the {\em layout of data structure} 
is the mapping of a data structure  into one or more memory spaces, typically with the intent of efficient and performant access by a collection of computations \note{not sure about this definition}.
The {\em execution policy} is wow a computation's units of work are scheduled and mapped onto data collections.
  Data decomposition? Data distribution ? control space or execution space? iteration space traversal (avoid using loop traversal, maybe use domain traversal?) 
...
We highlight the following key points when designing abstractions for data structures and layout:
...
\item TERMINOLOGY (define each of these up front)
 ...
 \item Data structure: A organized collection of datum residing in one or more memory spaces. 
  ...
  \note{Didem: the following ones are skipped for now
  } 
